"""Continuously play Jungle Hunt in the Boloup Android app."""
...
DEFAULT_X = 631
DEFAULT_Y = 1433
DEFAULT_COLLECT_X = 360
DEFAULT_COLLECT_Y = 1239
DEFAULT_COLLECT_DELAY = 0.2
DEFAULT_MIN_DELAY = 0.5
DEFAULT_MAX_DELAY = 2.0
DEFAULT_LOG_FILE = Path("artifacts/play_jungle_hunt_boloup/balance.log")
DEFAULT_OUTPUT_DIR = Path("artifacts/play_jungle_hunt_boloup")
DEFAULT_OCR_PATH = Path(r"D:\Program Files (x86)\Tesseract-OCR\tesseract.exe")
EXPECTED_SCREEN_SIZE = (720, 1640)
BALANCE_BOX = (220, 880, 500, 925)
BALANCE_SCALE = 4


class BalanceReadError(RuntimeError):
    """Raised when the current Jungle Hunt balance cannot be recognized."""
...
def read_balance(
    ...
    """Capture Jungle Hunt and return its current coin balance and OCR text."""
...
def append_balance_log(log_path, iteration, balance=None, raw_ocr="", error=""):
...
def run_spin_loop(
    ...
    """Dismiss a possible result overlay, spin, then record the balance."""
    validate_settings(
        x,
        y,
        collect_x,
        ...
        while iterations == 0 or click_count < iterations:
            ...
            if balance_reader is not None:
                try:
                    ...
                    print("余额识别：第 {} 次，余额={:,}".format(click_count, balance))
...
def build_parser():
    ...
    parser.add_argument("--serial", default=DEFAULT_SERIAL, help="ADB 设备序列号")
...
def main(argv=None):
    ...
    try:
        ...
        print(
            "开始 Boloup Jungle Hunt 自动点击：设备={}，兜底=({}, {})，"
        ...
        print("请保持 Jungle Hunt 页面位于前台；按 Ctrl+C 停止。")
